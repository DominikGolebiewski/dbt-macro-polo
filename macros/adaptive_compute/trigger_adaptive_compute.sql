...
{% macro default__trigger_adaptive_compute() %}
    ...
    {% if adaptive_config.get('enabled', false) and model_config.get('enabled', false) %}
        {{ dbt_macro_polo.log_event(message="Triggering adaptive compute", level='INFO', macro_name='trigger_adaptive_compute') }}
        {{ dbt_macro_polo.adaptive_compute() }}
    {% else %}
        {{ dbt_macro_polo.log_event(message="Adaptive compute not enabled", level='INFO', macro_name='trigger_adaptive_compute') }}
    {% endif %}
