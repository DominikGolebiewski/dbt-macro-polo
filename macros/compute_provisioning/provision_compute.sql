...
{% macro snowflake__provision_compute(incremental_size, fullrefresh_size=none) %}
    ...
    {% if state_value %}
        {{ dbt_macro_polo.logging(message="Allocated warehouse from runtime state", level='DEBUG', model_id=model_id, status=state_value | upper, macro_name=macro_name) }}
        {{ return(state_value) }}
    {% endif %}
    ...
    {{ dbt_macro_polo.logging(message="Saving warehouse '" ~ warehouse_id ~ "' to runtime state with key '" ~ state_key ~ "'", level='DEBUG', macro_name=macro_name) }}
    {% do macro_polo.get('runtime_state', {}).update({state_key: warehouse_id}) %}
    {{ dbt_macro_polo.logging(message="Allocated warehouse", model_id=model_id, status=warehouse_id | upper, macro_name=macro_name) }}
    {{ return(warehouse_id) }}
